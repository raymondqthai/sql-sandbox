...
-- Change default NULL string type to NULL data type 
INSERT INTO layoffs_staging2
SELECT
  NULLIF(company, 'NULL'),
  NULLIF("location", 'NULL'),
  NULLIF(industry, 'NULL'),
  NULLIF(total_laid_off, 'NULL'),
  NULLIF(percentage_laid_off, 'NULL'),
  NULLIF("date", 'NULL'),
  NULLIF(stage, 'NULL'),
  NULLIF(country, 'NULL'),
...
USING TRY_CAST("date" AS DATE);


-- Check Total Laid Off Column
SELECT total_laid_off
FROM layoffs_staging2;

-- Standardized Column to be Interger Type
ALTER TABLE layoffs_staging2
ALTER COLUMN total_laid_off TYPE INTEGER 
USING TRY_CAST(total_laid_off AS INTEGER);


-- Check Percentage Laid Off Column
SELECT percentage_laid_off
FROM layoffs_staging2;

-- Standardized Column to be Double Type
ALTER TABLE layoffs_staging2
ALTER COLUMN percentage_laid_off TYPE DOUBLE 
USING TRY_CAST(percentage_laid_off AS DOUBLE);


-- Check Funds Raised Millions Column
SELECT funds_raised_millions
FROM layoffs_staging2;

-- Standardized Column to be Integer Type
ALTER TABLE layoffs_staging2
ALTER COLUMN funds_raised_millions TYPE INTEGER 
USING TRY_CAST(funds_raised_millions AS INTEGER);
...
-- Standardized Empty Values with NULL
UPDATE layoffs_staging2
SET industry = NULL
WHERE industry = '';
